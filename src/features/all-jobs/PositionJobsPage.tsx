...
export default function PositionJobsPage({
  ...
  // 페이지네이션 계산
  const totalJobs = 234; // 직무별 전체 공고 수
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="relative">
              <select
                ...
                className="px-4 py-2 text-sm text-gray-700 transition bg-white border border-gray-300 rounded-lg hover:bg-gray-50 appearance-none pr-8 cursor-pointer"
              >
                  ...
                  {job.tags.length > 0 && (
                    <div className="flex gap-2 mb-3">
                      {job.tags.map((tag, idx) => (
                        <span
                          key={idx}
                          className="px-2 py-1 text-xs text-red-600 bg-red-50 rounded"
                        >
                          {tag}
                        </span>
                      ))}
