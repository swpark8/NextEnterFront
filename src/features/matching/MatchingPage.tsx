...
export default function MatchingPage({ onEditResume, initialMenu = 'matching' }: MatchingPageProps) {
  ...
  const handleConfirmAnalysis = () => {
    try {
      ...
      // 선택된 이력서와 공고 정보 가져오기
      const selectedResumeInfo = resumes.find(r => r.id.toString() === selectedResume);
      const selectedJobInfo = businessJobs.find(j => j.id.toString() === selectedJob);

      if (selectedResumeInfo && selectedJobInfo) {
        // 현재 날짜/시간
        const now = new Date();
        const date = now.toLocaleDateString('ko-KR').replace(/\. /g, '.').replace('.', '');
